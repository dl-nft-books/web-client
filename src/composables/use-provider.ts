...
import { TypedDataDomain, TypedDataField, providers } from 'ethers'
...
export const useProvider = () => {
  ...
  const isFallback = computed(() =>
    Object.values(FALLBACK_PROVIDERS).some(
      fallback => (fallback as string) === _providerReactiveState.providerType,
    ),
  )

  const currentProvider = computed(() =>
    isFallback.value
      ? (rawProvider.value as unknown as providers.JsonRpcProvider)
